...
describe('official TUI plugin', () => {
  ...
  it('runs a manual refresh when the command is selected', async () => {
    ...
    const refreshCommand = runtime.commands().find((command) => command.value === 'nim.refresh')
  ...
  it('wraps the host prompts and overrides prompt.submit for local /nim-refresh handling', async () => {
    ...
    const submitCommand = runtime.commands().find((command) => command.value === 'prompt.submit')
    ...
    ;(promptProps?.ref as ((ref: PromptRefLike | undefined) => void) | undefined)?.(promptRef)
  ...
  it('falls through to the host submit flow for non-refresh prompts', async () => {
    ...
    runtime.commands().find((command) => command.value === 'prompt.submit')?.onSelect?.()
